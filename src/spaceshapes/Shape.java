...
public abstract class Shape {
	...
	protected boolean _bouncedLeftRight = false;
	
	protected CarrierShape _container;
	...
	public void move(int width, int height) {
		...
		if (nextX <= 0) {
			nextX = 0;
			_deltaX = -_deltaX;
			_bouncedLeftRight = true; //has bounced off left wall
		} else if (nextX + _width >= width) {
			nextX = width - _width;
			_deltaX = -_deltaX;
			_bouncedLeftRight = true; //has bounced off right wall
		}

		if (nextY <= 0) {
			nextY = 0;
			_deltaY = -_deltaY;
			_bouncedLeftRight = false;
	...
	public void paint(Painter painter) {
		//Paint the shape using shapePaint (in the subclass)
		this.doPaint(painter);
		//Paint the text if there is any set to this shape
		if(_text != null) {
	...
	/*
	 * Set text that is to be painted in the shape
	 */
	 ...
	 * If the callee object is not a child within a CarrierShape instance this method returns null.
	 */
	...
	public List<Shape> path(){
		ArrayList<Shape> _path = new ArrayList<Shape>();
		//Go through each parent shape recursively and add them to the array
		if(_container != null) {
			_path.addAll(_container.path());
		}
		_path.add(this);
		return _path;
	}
